...
def findReviewInKakaoMapUrl(url) -> reviewData:
    ...
    try:
        while num < 3:
            try:
                num += 1
                review_text = review_text + driver.find_element(By.CSS_SELECTOR,
                ...
                if num == 3:
                    ...
                    reviewData.blog_review_qty = int(blog_review_qty)
                    reviewData.star_review_stars = float(star_review_stars)
                    reviewData.star_review_qty = int(star_review_qty.strip("("")"))
                    break
            except Exception as e1:
...
def initDriver():
    ...
    # Chrome 116버전 업데이트 -> Selenium 버전 업데이트 : 자동 드라이버 탐색 기능 추가
